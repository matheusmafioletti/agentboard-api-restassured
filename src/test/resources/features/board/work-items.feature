...
Feature: Work Item Management
  ...
  @smoke
  Scenario: Create TASK returns 201 with status NEW
  ...
  @high
  Scenario: Create USER_STORY returns 201 with status READY
    Given a FEATURE work item exists in the current project
  ...
  @high
  Scenario: Create FEATURE returns 201 with status BACKLOG
  ...
  Scenario: List work items returns 200
  ...
  @smoke
  Scenario: Update work item status via PATCH
    Given a FEATURE work item exists in the current project
    And I create a USER_STORY under the current feature
  ...
  Scenario: Create TASK with USER_STORY parent
    Given a FEATURE work item exists in the current project
    And I create a USER_STORY under the current feature
    When I create a TASK with parent USER_STORY
    Then the response status should be 201
  ...
  Scenario: Invalid hierarchy TASK parent of TASK returns 400
    Given a FEATURE work item exists in the current project
    And I create a USER_STORY under the current feature
    And I create a TASK under the current user story
  ...
  Scenario: Cross-tenant work item access returns 404
